...
# Extract the "Running on" IP/URL that is output in the log file generated by
# the crontab. Run `crontab -e` to change the file.
def extract_final_url(file_path):
    final_url = None
    with open(file_path, 'r') as file:
        for line in file:
            if line.startswith(" * Running on"):
                final_url = line.split()[-1]
...
if __name__ == "__main__":
    ...
    host = extract_final_url("/tmp/sigourney.log")
    if host != None:
        sigourney.send_notification(host)
